...
External repositories and target patterns resolve to nothing on purpose. The
first needs the repository mapping only Bazel can produce, and the second names
a set rather than a target, so either answer would be a guess.
