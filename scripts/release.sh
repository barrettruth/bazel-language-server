...
Pushed $tag.
The release workflow publishes to crates.io and attaches the binary from the tag.
EOF
  exit 0
fi
...
required_cst="$(sed -n 's/^starlark-cst = "\(.*\)"/\1/p' Cargo.toml | head -1)"
if [ -n "$required_cst" ]; then
  published_cst="$(
    cargo search starlark-cst --limit 1 2>/dev/null |
      sed -n 's/^starlark-cst = "\([^"]*\)".*/\1/p'
  )"
  [ -n "$published_cst" ] ||
    die "starlark-cst is not on crates.io; release it before releasing this crate"
  lowest_cst="$(printf '%s\n%s\n' "$required_cst" "$published_cst" | sort -V | head -1)"
  [ "$lowest_cst" = "$required_cst" ] ||
    die "Cargo.toml needs starlark-cst $required_cst but crates.io has $published_cst"
fi
