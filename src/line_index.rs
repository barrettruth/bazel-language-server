...
impl LineIndex {
    ...
    pub fn position(&self, text: &str, offset: usize) -> Position {
        ...
        let character = text.get(self.starts[line]..offset).map_or(0, utf16_len);
        #[allow(clippy::cast_possible_truncation)]
        Position {
            line: line as u32,
            character,
...
mod tests {
    ...
    fn counts_utf16_units() {
        ...
        assert_eq!(index.position(ASTRAL, 0), at(0, 0));
        assert_eq!(index.position(ASTRAL, 6), at(1, 0));
        ...
        assert_eq!(index.position(ASTRAL, x), at(1, 7));
    }
    ...
    fn round_trips_every_boundary() {
        ...
        for offset in (0..=ASTRAL.len()).filter(|o| ASTRAL.is_char_boundary(*o)) {
            let position = index.position(ASTRAL, offset);
            assert_eq!(index.offset(ASTRAL, position), offset, "offset {offset}");
        }
    }
    ...
    fn a_column_inside_a_surrogate_pair_rounds_down() {
        ...
        let start = index.offset(ASTRAL, at(1, 5));
        assert_eq!(index.offset(ASTRAL, at(1, 6)), start);
    ...
    fn clamps_past_the_end() {
        ...
        assert_eq!(index.position(text, 999), at(1, 0));
        assert_eq!(index.offset(text, at(99, 0)), text.len());
        // A column past the line stops before the newline.
        assert_eq!(index.offset(text, at(0, 99)), 1);
    }
    ...
    fn a_crlf_line_ends_before_its_terminator() {
        let text = "ab\r\ncd\n";
        let index = LineIndex::new(text);
        assert_eq!(index.offset(text, at(0, 99)), 2);
    }
}
